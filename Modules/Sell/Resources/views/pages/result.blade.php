    ...
    <div class="container mt-5">
        ...
        <form method="get">
            <div class="form-group">
                <label for="">Chọn giải đấu:</label>
                <select class="form-select-sm" name="league" onchange="this.form.submit()">
                    @foreach($leagues as $key => $league)
                        <option value="{{ $key }}" {{ selectedCompareValue($key, $query["league"] ?? "") }}>{{ $league["text"] }}</option>
                    @endforeach
                </select>
            </div>
        </form>
                                    ...
                                    <div class="col-4">
                                        <div class="d-flex justify-content-center">
                                            <span class="me-3 ms-3 pt-1 pb-1 ps-3 pe-3"
                                                  style="background-color: rgba(180,172,206,.28); color: #472f92">{{ $item["result"] }}</span>
                                        </div>
                                    </div>
                                    <div class="col-4">
